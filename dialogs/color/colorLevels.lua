...
local labelSliders = { "Inputs:", "Outputs:" }
...
dlg:combobox {
    ...
    onchange = function()
        ...
        local isa = channel == "A"
        local isb = channel == "B"
        local ist = channel == "Alpha"

        local bools = { isl, isa, isb, ist }
        ...
        while k < lenFrames do
            ...
            if srcCel then
                ...
                for hex, lab in pairs(hexToLabDict) do
                    -- Lightness.
                    local lTrg = lab.l
                    local lFac = lab.l * 0.01
                    local ly = remapChannel(
                        lFac,
                    ...
                    local aTrg = lab.a
                    if aViable then
                        local aFac = (lab.a - aMin) * aDenom
                        local ay = remapChannel(
                            aFac,
                    ...
                    local bTrg = lab.b
                    if bViable then
                        local bFac = (lab.b - bMin) * bDenom
                        local by = remapChannel(
                            bFac,
                    ...
                    local tFac = lab.alpha
                    local ty = remapChannel(
                        tFac,
